	...
	form.on('file', function(name, file) {
		...
		if(file.type.indexOf('image')<0){//文件类型不是合法的
		  ...
		  var n = new Date().getTime()+file.name.slice(file.name.lastIndexOf('.'));
		  	fs.renameSync(file.path, "./public/file/" + n);
		 	formFilePath = '/file/'+n;
		}
	});
	//流程正常处理
...
function addSql (fields,path,fn) {
	var query = require('./_mysql_query'),
		seStr = 'insert into newthing.activity(activitytitle,activitytime,activitycity,activitypersonnumber,activitytype,activitycost,activitypic,activitycontent,activityremark,userid) values("'+fields.title+'","'+fields.time+'","'+fields.city+'","'+fields.number+'","'+fields.type+'","'+fields.cost+'","'+path+'","'+fields.content+'","'+fields.remark+'","'+fields.userid+'")';
	//console.log(seStr)
